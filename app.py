...
MODEL_PATH = 'model/guava_effnet.pth'
DISEASE_JSON_PATH = 'guava_disease_info.json'
...
model.to(device)
model.eval()
print("🧠 Guava EfficientNet model loaded.")
...
def send_daily_report():
    if daily_stats["count"] == 0:
        return
    try:
        msg = Message("📊 Daily Click Report - Guava Disease Detection",
...
def send_prediction_result_email(filename, prediction_results, image_path=None):
    try:
        msg = Message("🍐 New Guava Disease Detection Result",
                      sender=app.config['MAIL_USERNAME'],
                      recipients=['cropai2025@gmail.com'])
        
        # Create email body with prediction results
        email_body = f"""
        🔍 Guava Disease Detection Result
...
def extract_disease_details(disease_data, label):
    ...
    telugu_water = disease_data.get("నీరు", water)
    ...
    return {
        ...
        "urdu": {
            ...
            "पानी": urdu_water,
            "دوا": urdu_medicine,
            "نامیاتی دوا": urdu_organic_medicine,
